...
const BASE =
  (
    document.querySelector("base")?.getAttribute("href") || "/pastebooks/"
  ).replace(/\/+$/, "") + "/";
...
$("#btnRegister")?.addEventListener("click", async () => {
  ...
  try {
    console.log("[register] POST v1/register");
...
$("#btnLogout")?.addEventListener("click", async () => {
  try {
    console.log("[logout] POST v1/logout");
    await api.logout();
  } finally {
    await boot();
  }
...
function renderTabs() {
  ...
  state.books.forEach((b) => {
    ...
    el.className =
      "tab" +
      (state.currentBook && b.id === state.currentBook.id ? " active" : "");
...
function openCharmDialog(existing) {
  ...
  sSel.innerHTML = shapes
    .map((s) => `<option value="${s}">${s}</option>`)
    .join("");
  cSel.innerHTML = colors
...
function shapePath(name) {
  switch (name) {
    case "square":
      return '<rect x="10" y="10" width="80" height="80" rx="8"/>';
    case "circle":
      return '<circle cx="50" cy="50" r="40"/>';
    case "triangle":
      return '<polygon points="50,10 90,90 10,90"/>';
    case "rectangle":
      return '<rect x="10" y="25" width="80" height="50" rx="8"/>';
    case "diamond":
      return '<polygon points="50,5 95,50 50,95 5,50"/>';
    case "heart":
      return '<path d="M50 86 L15 50 A20 20 0 1 1 50 30 A20 20 0 1 1 85 50 Z"/>';
    case "clover":
      return '<path d="M50 60 a15 15 0 1 1 0-30 a15 15 0 1 1 30 0 a15 15 0 1 1 -30 0 a15 15 0 1 1 0 30 Z"/>';
    case "spade":
      return '<path d="M50 10 C35 30,15 45,15 60 a15 15 0 0 0 30 0 a15 15 0 0 0 30 0 C85 45,65 30,50 10 Z M42 90 h16 v-10 h-16 Z"/>';
    case "hexagon":
      return '<polygon points="30,10 70,10 90,50 70,90 30,90 10,50"/>';
    case "star":
      return '<polygon points="50,8 60,38 92,38 66,56 76,86 50,68 24,86 34,56 8,38 40,38"/>';
    case "squiggle":
      return '<path d="M10 60 C 20 20, 60 20, 50 60 S 80 100, 90 60" stroke-width="0"/>';
    default:
      return '<rect x="15" y="15" width="70" height="70" rx="10"/>';
  }
}
...
async function selectBook(id) {
  ...
  $("#createdAt").textContent =
    "Created: " + new Date(state.currentBook.created_at).toLocaleString();
  $("#updatedAt").textContent =
    "Last edit: " + new Date(state.currentBook.updated_at).toLocaleString();

  // Respect base path for public link
  const link = `/api/public/books/${state.currentBook.id}`;
  $("#shareLink").innerHTML =
    `<a href="${link}" target="_blank">Public link</a>`;
...
async function boot() {
  try {
    ...
    if (state.userId) {
      ...
      $("#meId").textContent =
        state.userId.slice(0, 8) + (me.dev ? "… (dev)" : "…");
